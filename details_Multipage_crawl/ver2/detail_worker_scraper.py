# detail_worker_scraper.py (VERSION ĐÃ SỬA LỖI)
...
# --- 3. Tất cả các hàm Helper Parse Detail (ĐÃ SỬA LỖI CÚ PHÁP) ---
def parse_company_info_from_detail(soup_detail):
     ...
     if scale_item := container.find("div", class_="company-scale"): 
         if val_tag := scale_item.find("div", class_="company-value"): company_info['company_scale'] = val_tag.get_text(strip=True) # SỬA Ở ĐÂY
     if field_item := container.find("div", class_="company-field"):
         if val_tag := field_item.find("div", class_="company-value"): company_info['company_field'] = val_tag.get_text(strip=True) # SỬA Ở ĐÂY
     if address_item := container.find("div", class_="company-address"):
         if val_div := address_item.find("div", class_="company-value"): # SỬA Ở ĐÂY
...
def parse_general_job_info_from_detail(soup_detail):
     ...
     for group in box.find_all("div", class_="box-general-group"):
         if title_tag := group.find("div", class_="box-general-group-info-title"): # SỬA Ở ĐÂY
             if value_tag := group.find("div", class_="box-general-group-info-value"): # SỬA Ở ĐÂY
...
def parse_skills_categories_from_detail(soup_detail):
     ...
     for box in container.find_all("div", class_="box-category"):
         if title_tag := box.find("div", class_="box-title"): # SỬA Ở ĐÂY
             if tags_container := box.find("div", class_="box-category-tags"): # SỬA Ở ĐÂY
...
def main_worker():
    print("Bắt đầu Script 2: Worker xử lý chi tiết job...")
    
    driver = None # Khởi tạo driver là None
    try:
        # Khởi tạo Selenium
        options = uc.ChromeOptions()
        # options.add_argument("--headless") # Bỏ comment khi chạy ổn định
        ...
        while True: # Vòng lặp vô tận để liên tục kiểm tra và xử lý
            ...
            print(f"\n=> Đang xử lý Job ID: {job_id}, URL: {detail_url}")
            ...
            try:
                ...
                update_data = {
                    ...
                    **content_info,
                    ...
                    'status': 'completed' # Quan trọng: Đánh dấu là đã hoàn thành
                }
                ...
                conn.rollback() # Rollback lại transaction khi có lỗi
                # Đánh dấu là lỗi để không thử lại nữa
            ...
            sleep_duration = random.uniform(20, 45)
            print(f"   Nghỉ {sleep_duration:.1f} giây trước khi xử lý job tiếp theo...")
            time.sleep(sleep_duration)

    except KeyboardInterrupt:
        print("\nĐã nhận lệnh dừng. Kết thúc worker.")
    finally:
        # Dọn dẹp tài nguyên khi kết thúc (dù là do lỗi hay KeyboardInterrupt)
